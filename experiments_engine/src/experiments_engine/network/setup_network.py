...
def setup_ip_to_ip(delay: int|None=None, jitter: int|None=None, throttle_bw=None, packet_loss: float|None=None):
    ...
    run_except(["ip", "netns", "exec", Settings.namespace_1, "ip", "addr", "add", f"{Settings.ip_1}/24", "dev", Settings.veth_1])
    run_except(["ip", "netns", "exec", Settings.namespace_2, "ip", "addr", "add", f"{Settings.ip_2}/24", "dev", Settings.veth_2])
...
def decoder_endpoint():
    ...
    if Settings.protocol == "udp":
        ...
        return f"{Settings.protocol}://0.0.0.0:{Settings.port}?timeout=2000000"
    elif Settings == "rtsp":
